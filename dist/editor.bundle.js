...
<button class=${themeClass("button")} type=submit>${view.state.phrase("go")}</button>`;
    ...
    const gotoLine = view => {
        ...
        if (!panel) {
            view.dispatch({
                reconfigure: view.state.field(dialogField, false) == null ? { append: [panels(), dialogField, baseTheme$3] } : undefined,
    ...
    Prec.override(keymap.compute([snippetKeymap], state => state.facet(snippetKeymap)));
    EditorView.domEventHandlers({
    ...
    button1.addEventListener('click', () => {
      ...
      editorTransformed.state.doc.text.toString();
